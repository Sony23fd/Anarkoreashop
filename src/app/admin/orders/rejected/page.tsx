      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <ArchiveX className="w-6 h-6 text-red-500" />
            Цуцлагдсан захиалга
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Нийт <strong>{orders?.length || 0}</strong> цуцлагдсан барааны захиалга — <strong>{groups.length}</strong> бүлэг
          </p>
        </div>
          ...
          {groups.map((groupOrders) => {
            ...
            return (
              <div key={first.transactionRef || first.id}
                className={`bg-white rounded-xl border shadow-sm overflow-hidden border-l-4 border-l-red-400`}
              >
                {/* Customer Header */}
                ...
                {deliveryAddress && (
                  <div className="bg-slate-50 border-t px-5 py-2.5 text-sm text-slate-600 flex items-center gap-2">
                ...
                <div className="border-t px-5 py-4 flex items-center justify-between gap-4 bg-slate-50/60">
                  <p className="text-xs text-slate-400">
                    Энэ хэрэглэгчийн бүх захиалгыг буцааж сэргээх боломжтой
                  </p>
                  <GroupRejectedActions orderIds={groupOrders.map((o: any) => o.id)} />
                </div>
              </div>
            )
          })}
