...
export default async function DeliveredOrdersPage({ searchParams }: { searchParams: Promise<{ days?: string }> }) {
          ...
          {groups.map((groupOrders) => {
            ...
            return (
              <div key={first.transactionRef || first.id}
                className={`bg-white rounded-xl border shadow-sm overflow-hidden border-l-4 border-l-teal-400`}
              >
                {/* Customer Header */}
                ...
                <div className="border-t px-5 py-4 flex items-center justify-between gap-4 bg-slate-50/60">
                  <p className="text-xs text-slate-400">
                    Энэ хэрэглэгчийн ачаа амжилттай хүргэгдсэн байна
                  </p>
                  <GroupDeliveredActions orderIds={groupOrders.map((o: any) => o.id)} />
                </div>
              </div>
            )
          })}
